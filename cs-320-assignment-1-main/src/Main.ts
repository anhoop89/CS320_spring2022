...
export function reverse(list: List): List {
  ...
  if (list == null) {
    ...
    // With recursion, we traverse left-to-right **and** construct the list
    // left-to-right with snoc. 
    // Traverse from left to right until a null and copy the data from right to left
    // into a new list - make sure update a new rest.
    const output = reverse(list.rest);
    return snoc(list.first, output);
  }
}
...
export function removeZeroes(list: List): List {
  if (list == null) {
    ...
    const output = removeZeroes(list.rest);
    let check: List = list;
    if (check.first !== 0)      
       return cons(check.first, output);
    return output;
  }
}
...
export function countLeaves(tree: LogicTree): number {
  ...
  switch (tree.tag) {
    ...
    case "and":
      nodeCount =
      countLeaves(tree.leftSubtree) +
      countLeaves(tree.rightSubtree);
      break;
    case "not":
      nodeCount = countLeaves(tree.subtree);
      break;
    case "bool":
      nodeCount+= 1;
      break;
...
export function removeNots(tree: LogicTree): LogicTree {
  ...
  switch (tree.tag) {
    // if we meet "and" tag, we need to do recursively from the left to right and copy nodes 
    ...
    // if we meet "not", keep moving instead of copying left subtree or right subtree.
    case "not":
      return removeNots(tree.subtree);
    //if the node is "bool", that means it runs at the end of subtree, so we return a tree   
    case "bool":
      return tree;
  }
